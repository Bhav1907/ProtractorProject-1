...
export class CommonFunctions extends BasePage {
	...
	/* async db2queryHandler(conn_String: string, sql_Query: string, return_Data: boolean) {
		try {
			let conn = await ibmdb.open("Driver={IBM DB2 ODBC DRIVER};DATABASE=DBNAME;UID=USERNAME;PWD=PASSWORD;HOSTNAME=HOSTNAME;port=PORT");
		...
		switch (DBName.toLowerCase()) {
			...
			case 'db2':
				try {
					...
					//	let conn = await ibmdb.open("Driver={IBM DB2 ODBC DRIVER};DATABASE=DBNAME;UID=USERNAME;PWD=PASSWORD;HOSTNAME=HOSTNAME;port=POST");
